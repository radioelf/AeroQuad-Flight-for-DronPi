...
void resetBattery(byte batno) {
  ...
  if (batno < numberOfBatteries) {
    ...
    batteryData[batno].current      = 500000;
    batteryData[batno].maxCurrent   = 20;
...
void measureBatteryVoltage(unsigned short deltaTime) {
  ...
  for (int i = 0; i < numberOfBatteries; i++) {
	if (!current_read){
		ADC_voltage = (long)ads1115.readADC_SingleEnded(CHANNEL_V);// * batteryData[i].vScale;// / (1L<<ADC_NUMBER_OF_BITS) + batteryData[i].vBias;
		//Serial.print("ADC voltage: ");
		//Serial.println(ADC_voltage);
	}
	if (ADC_voltage >100 and ADC_voltage <10000) batteryData[i].voltage = ((ADC_voltage * (batteryData[i].vScale/100.0))/10.0);
#ifdef BM_EXTENDED
   ...
   if (current_read){
      //batteryData[i].current = (long)ads1115.readADC_SingleEnded(CHANNEL_I) * 0.56;//(batteryData[i].cScale / 1000000.0);// / (1L<<ADC_NUMBER_OF_BITS) + batteryData[i].cBias * 10;
      current_read = false;
   }else{
	  current_read = true;
   }
