...
#define GAIN_SRF08 0x08                                                 // Analog gain 1025 (default), min 0->94, max 31->1025 (31*70us = 2170us)
...
bool SRF08State;     
uint8_t rangerWaitCycles = 3;                                           // 3*20 =60ms
uint8_t DistanceSRF08Earth = 4;                                         // distancia en Cm del sensor al suelo
...
// change address I2C, (NEW_ADDRESS 8bits)
...
void inititalizeRangeFinders() {
  ...
  if (version !=255 && version !=0){	
	 if (GAIN_SRF08 !=0x1F){                                            // !default gain
	   Conf_Gain(GAIN_SRF08);
	   delayMicroseconds(70000);
     }
     if (RANGE_SRF08 !=0xFF){                                           // !default range
	   Conf_Range(RANGE_SRF08);
     }
     SRF08State = true;
     rangeFinderRange[0] = float (0.0);
  }else{
    SRF08State = false;
   }
}
...
void updateRangeFinders() {                                             // call 20 ms->50hz
  if (rangerWaitCycles == 3) SRF08_Command(CMS);                        // 60 ms
  if (rangerWaitCycles !=0) {
	  rangerWaitCycles--;
  }else{
	rangerWaitCycles = 6;                                               // 120 ms
	
	// int range = RX_SRF08()*10;                                       // mm
	int range = RX_SRF08();                                             // Cm
	if (range >DistanceSRF08Earth){
      range = (range - DistanceSRF08Earth);  
