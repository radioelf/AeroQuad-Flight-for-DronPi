...
#define SBUS_SYNCBYTE 0x0F // some sites say 0x0F
...
void initializeReceiver(int nbChannel = 10) {
  ...
  //One Byte = 1 startbit + 8 databit + 1 paritybit + 2 stopbit (8E2), baudrate = 100'000 bit/s 25 bytes
  #if defined (DuePi)
...
void readSBUS() {
  ...
  while(SERIAL_SBUS.available()) {
    ...
    if (sbusIndex == 25) {
      ...
      if (sbus[24] == 0x0) {
      // check stop bit(s) before updating buffers
      // The masked out bits are used for packet sequencing
      // and mode identification. They are not zero
      // for 12, 14 and 18-channel modes.
      //if ((sbus[24] & 0xC3) == 0 ) {
