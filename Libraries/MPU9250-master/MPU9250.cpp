...
bool MPU9250::init(bool calib_gyro, bool calib_acc){
    ...
    uint8_t MPU_Init_Data[MPU_InitRegNum][2] = {
        ...
        {my_low_pass_filter, MPUREG_CONFIG},     		// Use DLPF set Gyroscope bandwidth 42Hz
        #ifdef DPS250_2G
		...
		#endif
        {my_low_pass_filter_acc, MPUREG_ACCEL_CONFIG_2},// Set Acc Data Rates, Enable Acc LPF  ->0x03->00000011
...
#else
        {0x12, MPUREG_I2C_SLV0_DO},   					// Register value to 8Hz continuous measurement in 16bit (default)
#endif
        {0x81, MPUREG_I2C_SLV0_CTRL}  					// Enable I2C and set 1 byte
    ...
    for(i = 0; i < MPU_InitRegNum; i++) {
        ...
        delayMicroseconds(100000);  						// I2C must slow down the write speed, otherwise it won't work
    }
    #ifdef DPS250_2G
		set_acc_scale(BITS_FS_2G);                      // Accel scale +-2g 
		set_gyro_scale(BITS_FS_250DPS);  				// Gyro scale 250º/s
    #else
		set_acc_scale(BITS_FS_4G);        			    // Accel scale +-4g (4096LSB/g)
		set_gyro_scale(BITS_FS_1000DPS);                // Gyro scale 1000º/s
	#endif
	               
    #ifdef Magnetometer_MPU9250
      calib_mag();  
    #endif
    return 0;
}
...
void MPU9250::calib_mag(){
    ...
    //WriteReg(AK8963_CNTL1, MFS_16BITS << 4 | M_8HZ);
    WriteReg(AK8963_CNTL1, MFS_16BITS << 4 | M_100HZ);
    delayMicroseconds(50000);
...
void MPU9250::calibrate(float *dest1, float *dest2){  
    ...
    #ifdef DPS250_2G
        //WriteReg(MPUREG_CONFIG, 0x00);      // Set low-pass filter to 256 Hz
        WriteReg(MPUREG_CONFIG, 0x01);      // Set low-pass filter to 188 Hz
        //WriteReg(MPUREG_CONFIG, 0x02);      // Set low-pass filter to 98 Hz
    ...
    #else
        WriteReg(MPUREG_CONFIG, 0x03);      // Set low-pass filter to 42 Hz
		WriteReg(MPUREG_GYRO_CONFIG, 0x10);  // Set gyro full-scale to 1000 degrees per second, maximum sensitivity
		WriteReg(MPUREG_ACCEL_CONFIG, 0x08); // Set accelerometer full-scale to 4 g, maximum sensitivity
		uint16_t  gyrosensitivity  = 33;   // = 32.8 LSB/degrees/sec
    ...
    delayMicroseconds(40000); // accumulate 40 samples in 40 milliseconds = 480 bytes
