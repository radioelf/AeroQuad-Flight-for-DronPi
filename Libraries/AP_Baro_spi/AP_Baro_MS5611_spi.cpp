...
#include <SPI.h>
#include "AP_Baro_MS5611_spi.h"


/* on APM v.24 MS5661_CS is PG1 (Arduino pin 40) */
//#define MS5611_CS_PIN 40
...
uint8_t AP_Baro_MS5611::MS5611_SPI_read(byte reg)
{
  byte dump;
  uint8_t return_value;
  byte addr = reg; // | 0x80; // Set most significant bit
  digitalWrite(MS5611_CS_PIN, LOW);
...
uint16_t AP_Baro_MS5611::MS5611_SPI_read_16bits(byte reg)
{
...
uint32_t AP_Baro_MS5611::MS5611_SPI_read_ADC()
{
...
void AP_Baro_MS5611::MS5611_SPI_write(byte reg)
{
  byte dump;
  digitalWrite(MS5611_CS_PIN, LOW);
...
void AP_Baro_MS5611::init()
{
	...
	pinMode(MS5611_CS_PIN, OUTPUT);	 // Chip select Pin
