...
float nvrReadFloat(int address) {
  ...
  for (int i = 0; i < 2; i++) {
  #if defined(DuePi)
    floatOut.floatUShort[i]= readEEPROMI2C(address + 2*i);
  #else    
  ...
  for (int i = 0; i < 4; i++) {
  #if defined(DuePi)
    floatOut.floatByte[i]= readEEPROMI2C(address + i); 
  #else    
...
void nvrWriteFloat(float value, int address) {
  ...
  for (int i = 0; i < 2; i++) {
   #if defined(DuePi)
    writeEEPROMI2C(address + 2*i, floatIn.floatUShort[i]);
  #else    
  ...
  for (int i = 0; i < 4; i++) {
   #if defined(DuePi)
    writeEEPROMI2C(address + i, floatIn.floatByte[i]);
  #else    
...
long nvrReadLong(int address) {
  ...
  for (int i = 0; i < 2; i++) {
  #if defined(DuePi)
    longOut.longUShort[i] = readEEPROMI2C(address + 2*i);
  #else    
  ...
  #if defined(DuePi)
    longOut.longByte[i] = readEEPROMI2C(address + i);
  #else    
...
void nvrWriteLong(long value, int address) {
  ...
  for (int i = 0; i < 2; i++) {
  #if defined(DuePi)
    writeEEPROMI2C(address + 2*i, longIn.longUShort[i]);
  #else    
  ...
  for (int i = 0; i < 4; i++) {
  #if defined(DuePi)
    writeEEPROMI2C(address + i, longIn.longByte[i]);
  #else    
...
void initializeEEPROM() {
  ...
  // Range Finder
    ...
    for (uint8_t location = 0; location < MAX_WAYPOINTS; location++) {
...
void storeSensorsZeroToEEPROM() {
  ...
  writeFloat(accelOneG, ACCEL_1G_ADR);// nvrWriteFloat(accelOneG, GET_NVR_OFFSET(ACCEL_1G_ADR))
