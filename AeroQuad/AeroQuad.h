...
uint8_t previousFlightMode = ATTITUDE_FLIGHT_MODE;
#define TASK_100HZ 1
#define TASK_50HZ 2
#define TASK_10HZ 10
#define TASK_1HZ 100
...
uint8_t flightMode = RATE_FLIGHT_MODE;
unsigned long frameCounter = 0; // main loop executive frame counter
int minArmedThrottle; // initial value configured by user

float G_Dt = 0.002; 
int throttle = 1000;
uint8_t motorArmed = OFF;
uint8_t safetyCheck = OFF;
 ...
 */
 ...
 // special state that allows immediate turn off of Altitude hold if large throttle changesa are made at the TX
  uint8_t altitudeHoldState = OFF;  // ON, OFF or ALTPANIC
...
#define MAX_WAYPOINTS 16  // needed for EEPROM adr offset declarations
  ...
  GeodeticPosition missionPositionToReach = GPS_INVALID_POSITION;  // in case of no GPS navigator, indicate the home position into the OSD
    ...
    uint8_t navigationState = OFF;  // ON, OFF or ALTPANIC
    uint8_t positionHoldState = OFF;  // ON, OFF or ALTPANIC
 ...
 */
//#define DEBUG
uint8_t fastTransfer = OFF; // Used for troubleshooting
