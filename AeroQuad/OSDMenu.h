...
struct MenuItem {
  const uint8_t level;            // menu level the item is on
  const char *text;            // text to show
  void (*function)(byte,byte); // handler func on leaf level
  const uint8_t mode;             // data to give for handler function
};
...
uint8_t  menuInFunc = 0;       // tells if a handler func is active
                            // 0 - we're in base menu
                            // 1 - call handler on stick actions
                            // 2-255 countdown and callback when it gets to 1 - no sticks
uint8_t  menuEntry  = 255;     // Active menu entry
uint8_t  menuAtExit = 0;       // are we at the exit at the top
uint8_t  stickWaitNeutral = 1; // wait for stick to center
uint8_t  menuOwnsSticks = 0;   // menu code will handle stick input (prevents arming)

// DATA that menu functions can freely use to store state
uint8_t  menuFuncData[10];  // 10 bytes of data for funcs to use as they wish...
...
void menuHandleSimple(uint8_t mode, uint8_t action) {
...
void menuHandleConfirm(uint8_t mode, uint8_t action) {
  ...
  case MENU_DOWN:
    {
      ...
      uint8_t cpos = strlen(menuData[menuEntry].text) + 4;
...
#ifdef CameraControl
void menuHandleCam(uint8_t mode, uint8_t action) {
...
void menuHandleGoPro(uint8_t mode, uint8_t action) {
...
void menuEditFloat(float *f, uint8_t i, uint8_t d, uint8_t pos, uint8_t action, float min, float max) {
...
void menuHandlePidTune(uint8_t mode, uint8_t action) {
  ...
  if (menuFuncData[0] < 9) {
    ...
    uint8_t cl   = ((menuFuncData[0] > 6) ? 12 : 11) + menuFuncData[0];
    uint8_t cr   = cl;
    uint8_t updn = MENU_SYM_BOTH;
...
void menuSensorInfo(uint8_t mode, uint8_t action){
  switch (action) {
  case MENU_EXIT:
  case MENU_ABORT:
    menuInFunc=0;
...
void menuHideOSD(uint8_t mode, uint8_t action){
...
uint8_t  savedCameraMode;

// #define POWERSAVE 10 // enable to shut off servos after idle
#if defined (POWERSAVE)
  uint8_t idleCounter = POWERSAVE;
#endif

#define ZOOMPIN 24


void menuCameraPTZ(uint8_t mode, uint8_t action){
...
uint8_t  menuIsLast(uint8_t entry) {

  for (uint8_t i=entry+1; i<menuNumEntries; i++) {
...
void menuUp() {
  ...
  if ((0 == menuEntry) || (menuData[menuEntry].level > menuData[menuEntry-1].level)) {
    ...
    for (uint8_t i=menuEntry-1; i>=0; i--) {
...
void menuDown() {
  ...
  if (menuAtExit) {
    ...
    for (uint8_t i = menuEntry + 1; i < menuNumEntries; i++) {
      if (menuData[menuEntry].level == menuData[i].level) {
        menuEntry = i;
        break;
      }
